...
broker = 'test.mosquitto.org' # endereço IPv4 do broker na rede
port = 1883 # porta de utilização do MQTT, neste caso a padrão
client_id = "SMARTPHONE_YERRO" # como o dispositivo que executa este script irá se identificar no broker
...
dir_path = os.getcwd() + "/ArquivosPY/MQTTGlobal" # obtém o caminho do dirétorio de trabalho atual do python, e aponta para uma pasta específica dele para armazenamento dos txt's
print(dir_path)
...
def subscribe(client: mqtt_client):
    def on_message(client, userdata, msg): # função de retorno, que realiza manipulações com os dados recebidos dos tópicos inscritos
        ...
        if(msg.topic == "tfgyerro/dados"): # caso o dado seja do tópico 'dados', realiza salvamento dele em um txt próprio
        ...
        if(msg.topic == "tfgyerro/porcento"): # caso o dado seja do tópico 'porcentagem', realiza salvamento dele em um txt próprio
        ...
        if(msg.topic == "tfgyerro/reserva"): # caso o dado seja do tópico 'reserva', realiza o print dele
            print(msg.payload.decode() + "\n")

    client.subscribe("tfgyerro/dados",0) # inscrição no tópico 'dados', com a qualidade de serviço 0
    client.subscribe("tfgyerro/porcento",0) # inscrição no tópico 'porcento', com a qualidade de serviço 0
    client.subscribe("tfgyerro/reserva",0) # inscrição no tópico 'reserva', com a qualidade de serviço 0
